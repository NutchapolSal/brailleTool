...
const unicodeData = `2800;BRAILLE PATTERN BLANK;So;0;L;;;;;N;;;;;
...
export function createCodepointNameMap() {
	const map = new Map<number, string>();
	const lines = unicodeData.split('\n');
	for (const line of lines) {
		const [codepoint, name] = line.split(';');
		const codepointNum = parseInt(codepoint, 16);
		map.set(codepointNum, name);
	}
	return map;
}
